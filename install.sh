...
write_macos_ghostty_override() {
  local override_file shell_path shell_name shell_flag path_prefix

  override_file="$HOME/.config/ghostty-local/config"
  mkdir -p "$(dirname "$override_file")"

  if [[ -x "$HOME/.homebrew/bin/zsh" ]]; then
    shell_path="$HOME/.homebrew/bin/zsh"
    shell_name='zsh'
    shell_flag='-l'
    path_prefix="$HOME/.homebrew/bin:$HOME/.homebrew/sbin"
  elif [[ -x /opt/homebrew/bin/zsh ]]; then
    shell_path='/opt/homebrew/bin/zsh'
    shell_name='zsh'
    shell_flag='-l'
    path_prefix='/opt/homebrew/bin:/opt/homebrew/sbin'
